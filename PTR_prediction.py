...
def data_prep_melt(feature_path, ptr_path):
    ...
    ptr_long = ptr.melt(
        id_vars=id_vars, value_vars=value_vars, var_name="tissue", value_name="PTR"
    )
    ...
    cols_to_drop = [
        col
        for col in ["GeneName", "EnsemblTranscriptID", "EnsemblProteinID"]
...
def mixed_model(features, ptr_long):
    ...
    for train_idx, test_idx in tqdm(
        kf.split(merged), total=kf.get_n_splits(), desc="Cross-validation folds"
    ):

        merged_train = merged.iloc[train_idx]
        model = smf.mixedlm(
            formula, data=merged_train, groups=merged_train["EnsemblGeneID"]
        )
